...
from unblob.handlers.compression.uzip import UZIPExtractor, UZIPHandler
...
@pytest.mark.parametrize("toc", [[600, 500, 700], [600, 5000, 1000]])
def test_extract_rejects_unsorted_toc(toc, tmp_path: Path):
    # a descending offset makes `next - current` negative, which iterate_file
    # silently treats as zero, dropping the block instead of decompressing it.
    inpath = tmp_path / "in.uzip"
    inpath.write_bytes(build_image(toc))
    with pytest.raises(InvalidInputFormat):
        UZIPExtractor().extract(inpath, tmp_path)
