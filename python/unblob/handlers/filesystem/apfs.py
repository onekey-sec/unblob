...
# o_type for an NX container superblock: type=1, flags=OBJECT_TYPE_FLAGS_STORAGE_TYPE_PHYSICAL (bit 31)
_NX_OBJ_TYPE_NX_SUPERBLOCK = 0x80000001
...
class APFSHandler(StructHandler):
    ...
    def valid_header(self, header) -> bool:
        return header.o_type == _NX_OBJ_TYPE_NX_SUPERBLOCK
    ...
    def calculate_chunk(self, file: File, start_offset: int) -> ValidChunk | None:
        ...
        if not self.valid_header(header):
            raise InvalidInputFormat(
                f"APFS superblock o_type mismatch: {header.o_type:#x}"
            )
