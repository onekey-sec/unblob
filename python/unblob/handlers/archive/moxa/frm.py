...
class MoxaFRMExtractor(Extractor):
    ...
    def _extract_fs_section(
        ...
        for index in range(fs_header.file_count or fs_header.file_count2):
            ...
            # A file declared inside the filesystem section must keep its content
            # within that section. file_length comes straight from the entry, so an
            # oversized value would otherwise read past the section into the next
            # section (or to EOF) and write those bytes into the extracted file.
